...
mirrors     # update mirror list (reflector)
```
...
ehypr       # edit hyprland config directory
reload      # source .zshrc
```
...
hyprlog     # show last 50 lines of hyprland log
hyprpick    # color picker (copy to clipboard)
```
...
portcheck 3000         # shows process on port 3000
```
...
Zoxide remembers directories you visit. Use `z` instead of `cd`:
...
vim $(fzf)     # open file found by fzf
```
...
```bash
y              # open yazi
yy             # open yazi and cd into the directory you navigated to on exit
```
